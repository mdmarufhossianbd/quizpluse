...
const PopularQuize = async () => {
  ...
  return (
    <div className="lg:max-w-7xl xl:max-w-full mx-auto lg:px-20 px-5">
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5'>
        {quizes?.map((item, index) => {
          return (
            ...
            >
              <Image className="rounded group-hover:opacity-50 group-hover:scale-105 duration-200 w-full" src={item.imageUrl} alt={item.quizName} width={300} height={200}
              />
              ...
              <Button className='hidden group-hover:flex absolute left-[40%] top-[40%]'>Start Now</Button>
            </div>
          );
        })}
      </div>
